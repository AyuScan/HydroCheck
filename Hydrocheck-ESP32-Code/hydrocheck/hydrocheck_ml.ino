/*
  HYDROCHECK - Full Prototype with TinyML Integration
  ...
  Libraries:
    ...
    EloquentTinyML (For running TensorFlow Lite model on ESP32)
*/
...
// Include TinyML inference module
...
void sampleAndProcess() {
  ...
  // ---- Heat strain accumulator ----
  if (calibrated) {
    float tempRisk = constrain((skinTemp - baselineSkinTemp) * 8.0, 0, 35);
  ...
  // ---- TinyML Data Feed & Inference ----
  float probLowRisk = 1.0f;
  float probModRisk = 0.0f;
  int predictedClass = 0;

  if (calibrated) {
    // 1. Feed normalized features into the rolling window
    DehydrationML::addSample(
      filteredGSR,
      gsrRateOfChange,
      bpm,
      hrv,
    ...
    // 2. Predict once the window has filled (requires 10 samples)
    if (DehydrationML::bufferCount >= DehydrationML::WINDOW_SIZE) {
      predictedClass = DehydrationML::predict(probLowRisk, probModRisk);
      Serial.printf("[ML] Inference Output - Low: %.2f, Mod/High: %.2f (Class: %d)\n",
                    probLowRisk, probModRisk, predictedClass);
    } else {
      Serial.printf("[ML] Window buffer filling: %d/%d\n",
                    DehydrationML::bufferCount, DehydrationML::WINDOW_SIZE);
    }
  }

  // ---- Blended risk mapping ----
  // We use the predicted probability of moderate/dehydration risk to calculate the index
  float riskIndex = probModRisk * 100.0f;
  ...
  // ---- Serial debug ----
  Serial.printf("GSR:%.1f dGSR:%.3f BPM:%.0f HRV:%.1f SkinT:%.1f Strain:%.1f MLRisk:%.1f Calib:%d\n",
  ...
  if (bleConnected) {
    ...
    snprintf(payload, sizeof(payload),
             "{\"risk\":%.1f,\"hr\":%.0f,\"skin\":%.1f,\"gsr\":%.0f,\"strain\":%.1f,\"ml_class\":%d}",
             riskIndex, bpm, skinTemp, filteredGSR, heatStrainScore, predictedClass);
